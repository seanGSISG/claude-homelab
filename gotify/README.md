...
Gotify uses centralized `.env` file at `~/workspace/homelab/.env` for credential management.
...
→ Check that `GOTIFY_URL` and `GOTIFY_TOKEN` are set in `~/workspace/homelab/.env`
